        ...
        if dialog.chat.type in ['group', 'supergroup', 'channel']:
            if dialog.chat.is_creator:
                if dialog.chat.username:
                    c = f"[{dialog.chat.title}](https://t.me/{dialog.chat.username}) (public) [`{dialog.chat.id}`]\n"
                else:
                    c = f"[{dialog.chat.title}]({(await m.client.get_chat(dialog.chat.id)).invite_link}) (private) [`{dialog.chat.id}`]\n"
...
async def admin(m: Message):
    ...
    async for dialog in m.client.iter_dialogs():
        if dialog.chat.type in ['group', 'supergroup', 'channel']:
            try:
                if (await m.client.get_chat_member(dialog.chat.id, m.client.id)).status == 'administrator':
                    if dialog.chat.username:
                        c = f"[{dialog.chat.title}](https://t.me/{dialog.chat.username}) (public) [`{dialog.chat.id}`]\n"
                    else:
                        c = f"[{dialog.chat.title}]({(await m.client.get_chat(dialog.chat.id)).invite_link}) (private) [`{dialog.chat.id}`]\n"
